...
import { Plane, BarChart2, Users, Ghost, AlertTriangle, FileText, Bot, Activity, ChevronRight } from "lucide-react";
...
const HUB_STATUS = [
  { code: "ATL", status: "normal" },
  { code: "DTW", status: "normal" },
  { code: "MSP", status: "warning" },
  { code: "SLC", status: "normal" },
  { code: "SEA", status: "normal" },
  { code: "LAX", status: "normal" },
  { code: "JFK", status: "warning" },
  { code: "BOS", status: "normal" },
];
...
export default function Sidebar({ activeTab, onTabChange, filters, onFiltersChange, quickStats }: SidebarProps) {
  return (
          ...
          {HUB_STATUS.map((hub) => (
            <div
              ...
              className={`text-center py-1 px-1.5 rounded text-xs font-medium ${
                hub.status === "warning" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"
              }`}
            >
              {hub.status === "warning" ? "🟡" : "🟢"} {hub.code}
            </div>
          ))}
        </div>
      </div>
